                    ...
                    <div className="grid bg-[#8CE0FF] w-full max-w-7xl gap-10 rounded-2xl py-10 overflow-auto relative">
                        ...
                        <div className="flex flex-col md:flex-row justify-between px-4 md:px-10 gap-6">
                            <div className="bg-white rounded-xl p-6 w-full md:w-[45%]">
                                ...
                                <div className="mb-4">
                                    <p className="text-lg mb-2">Impostos: R$ {herdeiTaxes.toLocaleString()}</p>
                                    <ProgressBar percentage={herdeiTaxesPercent} />
                                    <p className="text-lg mb-2 mt-4">Cartório: R$ {herdeiNotary.toLocaleString()}</p>
                                    <ProgressBar percentage={herdeiNotaryPercent} />
                                    <p className="text-lg mb-2 mt-4">Honorários: R$ {herdeiFees.toLocaleString()}</p>
                                    <ProgressBar percentage={herdeiFeesPercent} />
                                    <p className="text-lg font-semibold mt-4">Total: R$ {herdeiTotal.toLocaleString()}</p>
                            ...
                            <div className="bg-white rounded-xl p-6 w-full md:w-[45%]">
                                ...
                                <div className="mb-4">
                                    <p className="text-lg mb-2">Impostos: R$ {othersTaxes.toLocaleString()}</p>
                                    <ProgressBar percentage={othersTaxesPercent} />
                                    <p className="text-lg mb-2 mt-4">Cartório: R$ {othersNotary.toLocaleString()}</p>
                                    <ProgressBar percentage={othersNotaryPercent} />
                                    <p className="text-lg mb-2 mt-4">Honorários: R$ {othersFees.toLocaleString()}</p>
                                    <ProgressBar percentage={othersFeesPercent} />
                                    <p className="text-lg font-semibold mt-4">Total: R$ {othersTotal.toLocaleString()}</p>
                                    <ProgressBar percentage={100} />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row gap-4 justify-center h-15 mt-8 w-full max-w-7xl mx-auto px-4 md:px-10">
                            <button
                                onClick={() => window.location.href = "https://api.whatsapp.com/send/?phone=5511941062025&text=Quero+falar+com+um+Especialista&type=phone_number&app_absent=0"}
