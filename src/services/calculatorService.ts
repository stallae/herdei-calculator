import {
  STATE_TAX,
  REGISTRY_TAXES,
  MaritalStatusDiscount,
} from "../configuration/CalculatorConstants";
import {
  MaritalStatusType,
  LocationType,
...
export const calculateTaxes = async (formData: FormData): Promise<CalculationResult> => {
  ...
  goods.forEach((good) => {
    ...
    const discount = MaritalStatusDiscount(
      maritalStatus,
...
function calculateRegistryFee(value: number, type: string): number {
  ...
  for (let i = 0; i < ranges.length - 1; i++) {
    if (value >= ranges[i] && value < ranges[i + 1]) {
      fee =
        REGISTRY_TAXES.VALUE[ranges[i] as keyof typeof REGISTRY_TAXES.VALUE];
      break;
    }
  }
  ...
  if (value >= ranges[ranges.length - 1]) {
    fee =
      REGISTRY_TAXES.VALUE[
        ranges[ranges.length - 1] as keyof typeof REGISTRY_TAXES.VALUE
      ];
  }
...
function calculateITMCDTaxRateByState(state: string, value: number): number {
  ...
  for (let i = brackets.length - 1; i >= 0; i--) {
    if (value >= brackets[i]) {
      rate =
        stateTax.ITCMD[brackets[i].toString() as keyof typeof stateTax.ITCMD];
      break;
    }
  }
